...
def paletteMaker(image_array):
    ...
    while (len(colors) >= 70) or (len(colors) == 0):
        colors = [[0, 0, 0, 0]]  # Default transparency color
        it += 1
        if it == 2:  
            round_value = 0
        round_value += 5
        ...
        for i in range(height):
            for j in range(width):
                pix = image_array[i, j]  # Access pixel data directly
                color = [0, 0, 0, 0]
                ...
                for a in range(4):  
                    if pix[a] == 255:
                        ...
                        color[a] = pix[a] - (pix[a] % round_value)  # Round color values
                
                if color[3] != 0 and color not in colors:  # Ignore duplicate transparent colors
                    colors.append(color)
                    
    return [[format(color[3], '08b')] + [format(value, '08b') for value in color[:3]] for color in colors], round_value
    ...
    for yidx in range(math.ceil(img_height / split)):  # y index of subimage
        for xidx in range(math.ceil(img_width / split)):  # xindex of subimage
            ...
            if sub_img_array.shape[0] != split or sub_img_array.shape[1] != split:
                continue
            palette, round = paletteMaker(sub_img_array)
            if palette != []:
