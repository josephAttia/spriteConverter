...
def upload(event):
    ...
    if imagefile == '':   #Check if image was selected
        return 'break'
    image = Image.open(imagefile)   #Open image
    width, height = image.size   #Save image width and height
    palletteMaker(image)   #Generate full image color palette
    image_padded = Image.new(image.mode, (3*width, 3*height), (255,255,255,0))   #Create transparent image 3x size of original
    ...
    SetLines()
    return "break"
def palletteMaker(image):
    global colors
    global round
    round = -4
    colors = []
    it = 0
    while (len(colors) >= 70) or (len(colors) == 0):
        colors = ['00000000_00000000_00000000_00000000']  # Include default transparency color
        it += 1
        if it == 2:  # Make sure rounds follow multiples of 5 after the initial value of 1
            round = 0
        round += 5
        for i in range(image.size[1]):
            for j in range(image.size[0]):
                pix = image.getpixel((j, i))  # Retrieve pixil data (left to right)
                color = [0, 0, 0, 0]
                for a in range(0, 4):  # Make sure maximum color values aren't rounded down
                    if pix[a] == 255:
                        color[a] = 255
                    else:  # Round color values
                        color[a] = pix[a] - (pix[a] % round)
                binary = f'{color[3]:08b}' + '_' + f'{color[0]:08b}' + '_' + f'{color[1]:08b}' + '_' + f'{color[2]:08b}'  # Convert color from integer to binary
                if color[3] != 0:  # Remove duplicate transparents
                    if binary not in colors:  # Remove duplicate colors
                        colors.append(binary)
def byteMaker():
    global s_list
    global layout
    global layout_arr
    ...
    declaration = ''
    layout = '}'
    arr = [[None]*math.ceil(cropped.size[0]/split) for i in range(math.ceil(cropped.size[1] / split))]
    s_list = []
    for yidx in range(math.ceil(cropped.size[1] / split)):  # y index of subimage
        for xidx in range(math.ceil(cropped.size[0] / split)):  # xindex of subimage
            num += 1
            s = ''
            for i in range(yidx * split, (yidx + 1) * split):
                for j in range(xidx * split, (xidx + 1) * split):
                    color = [0, 0, 0, 0];
                    if i < cropped.size[1] and j < cropped.size[0]:  # Check if subimage is out of original image bounds
                        pix = cropped.getpixel((j, i))  # Retrieve pixel data (left to right) of subimage
                        for a in range(0, 4):  # Make sure maximum color values aren't rounded down
                            if pix[a] == 255:
                                color[a] = 255
                            else:  # Round color values
                                color[a] = pix[a] - (pix[a] % round)
                    binary = f'{color[3]:08b}' + '_' + f'{color[0]:08b}' + '_' + f'{color[1]:08b}' + '_' + f'{color[2]:08b}'  # Convert color from integer to binary
                    if color[3] == 0:  # Force all transparents to be the same byte data
                        binary = '00000000_00000000_00000000_00000000'
                    idx = colors.index(binary)  # Match pixel color to index in color palette
                    if j == xidx * split:  # Closing brace if at the end of a row
                        s += "}"
                    if idx < 16:  # Include zero before single digit index
                        s += '$0' + str(hex(idx))[2:] + ','
                    else:
                        s += '$' + str(hex(idx))[2:] + ','
                    if j == (xidx + 1) * split - 1:  # Brace formatting after final pixel
                        if i == (yidx + 1) * split - 1:
                            s = s[:len(s) - 1]
                        s += "{\n"
            s += "}\n"
            flag = False
            for val in ['1', '2', '3', '4', '5', '6', '7', '8', '9', 'a', 'b', 'c', 'd', 'e',
                        'f']:  # Prevent inclusion of transparents sub sprites
                if val in s:
                    flag = True
            if flag:
                if s in s_list:  # Prevent inclusion of repeat sub sprites
                    ...
                    layout += str(s_list.index(s)+startnum) + ",\t"
                    arr[yidx][xidx] = s_list.index(s) + startnum
                if yidx == math.ceil(cropped.size[1] / split)-1 and xidx == math.ceil(cropped.size[0] / split)-1:
                  layout = layout[:-2]
            else:
                layout += "--\t"
        layout += "{\n}"
...
def convert(event):
    ...
    for i in range(len(s_list)):
        ...
        bytedata += s_list[i]
        declaration += "\tdebug(`screen spritedef " + str(i+startnum) + " " + str(split) + " " + str(split) + " `uhex_byte_array_(@" + textname + "_" + str(i+startnum) + ", " + str(split * split) + ") `uhex_long_array_(@" + textname + ", " + str(len(colors)) + "))\n"
    longdata = "con '" + textname + " sprite colors\n" + "dat\n"
    longdata += textname + " long {\n"
    for i in range(len(colors)):
        if i == len(colors) - 1:
            longdata += "}%" + colors[i] + "   'index=" + str(hex(i)) + "  -  " + str(i) + "\n"
        else:
            longdata += "}%" + colors[i] + ",{ 'index=" + str(hex(i)) + "  -  " + str(i) + "\n"
...
logButton = make_button(root, int((frameSize[0]/12))-int(frameSize[0]/11.25/2), 5+int(frameSize[1]/8)+int(frameSize[1]/15)+15+int(frameSize[1]/20)+15+int(frameSize[1]/15)+(15*19)+5+int(frameSize[1]/15)+15, int(frameSize[1]/30), int(frameSize[0]/11.25), text='save log', bg='silver', font='Terminal 8')
